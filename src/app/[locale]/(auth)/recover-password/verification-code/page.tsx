  ...
  const onSubmit: SubmitHandler<VerificationCodeFormInputs> = async (data) => {
    try {
      ...
      if (response.status === 202) {
        ...
        const codigoEntidade = response.data.info.codigo_entidade;

        localStorage.setItem(
          "entidade_esqueci_senha",
          JSON.stringify(codigoEntidade)
        );

        router.push("/recover-password/reset-password");
      }
        ...
        <div className="flex flex-col w-full">
          ...
          <p className="text-gray-400">
            ...
            <span className="text-primary font-medium">
              {verificationCodeInfo && verificationCodeInfo.email}
            </span>
          </p>
        </div>
